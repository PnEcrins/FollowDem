...
class animals 
{
	...
	*/
	...
	{
		$db=db::get();
		$rqs = $db->prepare('SELECT id_animal,ani_name,active,creation_date,update_date FROM '.config::get('db_prefixe').'animal where id_animal = ?');
		$rqs->execute(array($this->id));
		
		if($results = $rqs->fetchObject())
		{
			$this->set_nom($results->ani_name);
			

			$this->set_objects_features(objects_features::load_all($this->get_id()));
			
			$this->set_active($results->active);
			$this->set_date_creation($results->creation_date);
			$this->set_date_maj($results->update_date);
	...
	*/
	...
	private function save()
	{
		...
		trace::add("gps_data animals maj/ajoutée id=".$this->get_id().'--'.$this->get_nom());
	}
	...
	*/
	...
	{
		$db=db::get();
		$rqe = $db->prepare('SELECT count(id) as nb,nom,date_maj FROM '.config::get('db_prefixe').'animals where id = ?');
	...
	*/
	...
	{
		$db=db::get();
		$rqi = $db->prepare('INSERT INTO '.config::get('db_prefixe').'animals VALUES(?,?,?,?,?)');
		$rqi->execute(array($this->get_id(),$this->get_nom(),$this->get_date_creation_bdd(),$this->get_date_maj_bdd(),$this->get_active()));
		
		trace::add("Ajout animals id=".$this->get_id().'--'.$this->get_nom());
	}
	...
	*/
	...
	{
		$db=db::get();
		$rqu = $db->prepare('UPDATE '.config::get('db_prefixe').'animals SET nom=?,date_maj=?,active=? WHERE id=?');
		$rqu->execute(array($this->get_nom(),$this->get_date_maj_bdd(),$this->get_active(),$this->get_id()));
		if ($rqu->rowCount() === 0){
			trace::add('ERREUR (<pre>'.print_r($rqu->errorInfo(),true).'</pre>) update animals id='.$this->get_id().'--'.$this->get_nom(),$this->get_date_maj_bdd(),$this->get_active());
		}
		else {		
			trace::add("update animals id=".$this->get_id().'--'.$this->get_nom(),$this->get_date_maj_bdd(),$this->get_active());
		}
	}
	/**
	* 	Delete un Objet
	...
	*/
	...
	{
		$db=db::get();
		$rqd = $db->prepare('DELETE FROM '.config::get('db_prefixe').'animals WHERE id=?');
		$rqd->execute(array($this->get_id()));
		trace::add("Suppression animals id=".$this->get_id().'--'.$this->get_nom());
		if($cascade===true)
		{
			trace::add("Suppression gps_data animals id=".$this->get_id().'--'.$this->get_nom());
			foreach($gps_data as $data)
				$data->delete();
			
			trace::add("Suppression objects_features animals id=".$this->get_id().'--'.$this->get_nom());
	...
	*/
	...
	static function load_all($order='id_animal',$load_gps_data='load_gps_data_last')
	{	
		$db=db::get();
		$tmp_animals = array();

		
		$rqs = $db->prepare('SELECT id_animal FROM '.config::get('db_prefixe').' animal WHERE active=1 ORDER BY '.$order);			
		$rqs->execute();
		...
		while($result = $rqs->fetchObject())
		{
			...
			if(gps_data::load_all_by_date($result->id_animal,$d->format('Y-m-d H:m:i'),date('Y-m-d H:m:i'),false,true)!== false)
			{
				$tmp_animals[$i] = new animals($result->id_animal);
				$tmp_animals[$i]->$load_gps_data();
				$i++;
			}
		}
		return $tmp_animals;
	...
	* 	Fichier placé dans le répertoire csv/animals.csv obligatoirement (traitement en CRON possible - Cf controleur).
	*
	* 	@access  public
	* 	@return  none
	*/
		...
		if (($fs = fopen(config::get('csv_repertoire').config::get('system_separateur').config::get('csv_name'), "r")) !== FALSE) 
		{
			...
			while (($data = fgetcsv($fs,0,$delimiter,$enclosure)) !== FALSE) 
			{
				...
				if($result_cond)
				{
					...
					$tmp_animals = new animals();
					$tmp_animals->set_nom($data[config::get('csv_colonne','nom')]);
					$tmp_animals->set_id($data[config::get('csv_colonne','id')]);
					...
					if(isset($last_date_animals[$tmp_animals->get_id()]))
					{
						if($last_date_animals[$tmp_animals->get_id()] <  $obj_date->format('Y-m-d H:i:s'))
						{
							$last_date_animals[$tmp_animals->get_id()] = $obj_date->format('Y-m-d H:i:s');
							$tmp_animals->set_date_maj($obj_date->format('Y-m-d H:i:s'));
						}
					}
					else
					{
					...
					$gps_data		->set_id_animal($tmp_animals->get_id());
					...
					$tmp_animals	->set_gps_data(array($gps_data));
					...
					foreach(config::get('csv_colonne_objects_features') as $clef=>$value)
					{
						...
						$object_feature->set_id_animal($tmp_animals->get_id());
						$object_feature->set_nom_prop($clef);
						$object_feature->set_valeur_prop($value);
						$tmp_objects_features[$clef] = $object_feature;
					}
					$tmp_animals->set_objects_features($tmp_objects_features);
					
					echo $line.'animals sauvardé :'.print_r($tmp_animals);
			...
			fclose($fs);
			echo $line.'Dernière date de capture sauvées :'.print_r($last_date_animals);
	...
	*
	*/
	static function get_couleur_name(animals $bouquetins)
	{
		...
		if(isset($tmp[1]) && isset($tmp[2]))
		{
			...
			$couleurD	->set_id_animal($bouquetins->get_id());
			...
			$couleurG->set_id_animal($bouquetins->get_id());
