...
class Animal
{
    ...
    static function load_all($order='name')
    {
        $db=db::get();
        $tmp_animals = array();
            $rqs = $db->prepare('SELECT * FROM '.config::get('db_prefixe').'t_animals WHERE active = TRUE AND id_animal in ( select id_animal from '.config::get('db_prefixe').'cor_animal_devices)  ORDER BY '.$order);
            $rqs->execute();
