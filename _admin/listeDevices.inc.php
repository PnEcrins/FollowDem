...
if(isset($_GET['btSupprimer'])){
	$reqSupprObj = $db->prepare('DELETE FROM '.config::get('db_prefixe').'devices where id_device = :id');
	$reqSupprObj->execute(array('id' => $_GET['btSupprimer']));
	$reqSupprProp = $db->prepare('DELETE FROM '.config::get('db_prefixe').'devices where id_device = :id_device');
	$reqSupprProp->execute(array('id_device' => $_GET['btSupprimer']));
}
...
if (isset($_GET['btSuivant'])){
	...
	$cpt = $nbDepart + 15;
	$db=db::get();
	$reqObj = $db->prepare('SELECT * FROM '.config::get('db_prefixe').'devices LIMIT '.$cpt.',15');
	$reqObj->execute();
	...
	$resultObj = $reqObj->fetchAll();
		...
		$db=db::get();
		$reqObj = $db->prepare('SELECT * FROM '.config::get('db_prefixe').'devices LIMIT 0,15');
		$reqObj->execute();
		$resultObj = $reqObj->fetchAll();
	}
}
...
<div id="decale">
	<div class="row">
		...
		<div class="col-md-4">
			<h3>Contenu de la table 'devices'</h3>
		</div>
	...
	<table class="table table-striped table-bordered table-hover table-condensed">
		<tr><th></th><th></th><th></th><th>Id Device</th><th>Ref Device</th><th>Device Type</th><th>Device Info</th></tr>
		<?php foreach($resultObj as $row){ ?>
			<tr>
				<td>
					<form name="formSuppr" action="listeDevices.inc.php" method="GET"><button type="submit" class="btn btn-default" name="btSupprimer" value="<?php echo $row['id']; ?>"><span class="glyphicon glyphicon-trash"></span></button></form>
				</td>
				<td>
				...
				<td><?php echo $row['ref_device']; ?></td>
                <td><?php echo $row['id_device_type']; ?></td>
                <td><?php echo $row['ref_device']; ?></td>
			</tr>
		<?php } ?>
	</table>
